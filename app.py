    ...
    <script type="text/babel">
        ...
        const itinerary = [
             ...
             { day: 3, date: "4/19 (日)", title: "淺草與晴空塔", events: [ 
                 ...
                 { time: "19:30", title: "返回飯店", desc: "回程", icon: "🏨", location: HOTEL_ADDRESS, transport: { route: "田原町站 → 上野 → 飯店", line: "銀座線 直達轉步行", time: "15分" }, hideMap: true,
                   stationGuide: {
                     name: "返回飯店 (淺草出發)", desc: "搭乘銀座線至上野後步行",
                     tips: ["從田原町站上車最近", "回到熟悉的上野站再散步回飯店"],
                     routes: ["吃飽後步行至 ROX 商場附近的「田原町站」", "搭乘「銀座線(黃色)」往澀谷方向", "搭乘 2 站至「上野站」下車", "出站後沿著淺草通往東直走，步行約 10 分鐘即可抵達飯店"]
                   }
                 } 
             ] },
        ...
        const travelInfos = [
            { 
                title: "成田機場 T1", icon: "✈️", 
                items: [
                    "【入境】抵達 T1 位於南翼 (South Wing)。拿完行李後請至 B1 鐵道大廳購買交通票券。",
                    "【交通】B1 可搭乘京成 Skyliner 直達上野，亦可購買兒童版西瓜卡。",
                    "【出境】報到櫃檯位於 4F 出境大廳。出關後的免稅店位於 3F，建議把伴手禮留到最後免稅店再買，方便直接手提上機。"
                ]
            },
            { 
                title: "上野車站與置物櫃", icon: "🚉", 
                items: [
                    "【京成上野站】專搭 Skyliner，與 JR 上野站不共構，需出站走地面或地下聯通道。",
                    "【JR 上野站】搭新幹線需進「中央改札」，再搭手扶梯深入 B3/B4 月台。",
                    "【🧳 行李寄放】京成上野站的改札口外有大量大型置物櫃 (Coin Lockers)，可刷 Suica，適合最後一天前往機場前寄放。若客滿，JR 上野站淺草口或不忍口附近亦有許多置物櫃與人工行李寄存處。"
                ]
            },
            { 
                title: "東京地鐵轉乘", icon: "🚇", 
                items: [
                    "【看顏色與代號】例如：銀座線是黃色G、日比谷線是銀色H。跟著沿路同顏色的指標走即可。",
                    "【兒童卡】小孩刷實體兒童版交通卡過閘門時，機器會發出「嗶嗶」兩聲小鳥叫，代表成功扣除半價兒童票。",
                    "【乘換案內】使用 Google Maps 時，務必確認月台編號與行駛方向（例如：往淺草、往澀谷），避免搭反。"
                ]
            }
        ];
        ...
        const japanesePhrases = [
            {
                category: "📍 重要地點",
                words: [
                    { zh: "成田機場", ja: "成田空港", romaji: "Na-ri-ta Kū-kō" },
                    { zh: "上野", ja: "上野", romaji: "U-e-no" },
                    { zh: "淺草", ja: "浅草", romaji: "A-sa-ku-sa" },
                    { zh: "輕井澤", ja: "軽井沢", romaji: "Ka-ru-i-za-wa" },
                    { zh: "澀谷", ja: "渋谷", romaji: "Shi-bu-ya" },
                    { zh: "新宿", ja: "新宿", romaji: "Shin-ju-ku" },
                    { zh: "豐洲", ja: "豊洲", romaji: "To-yo-su" }
                ]
            },
            {
                category: "🗣️ 實用會話",
                words: [
                    { zh: "你好", ja: "こんにちは", romaji: "Kon-ni-chi-wa" },
                    { zh: "謝謝", ja: "ありがとう", romaji: "A-ri-ga-tō" },
                    { zh: "不好意思 / 請問", ja: "すみません", romaji: "Su-mi-ma-sen" },
                    { zh: "洗手間在哪裡？", ja: "トイレはどこですか？", romaji: "To-i-re wa do-ko de-su ka?" },
                    { zh: "這個多少錢？", ja: "これはいくらですか？", romaji: "Ko-re wa i-ku-ra de-su ka?" },
                    { zh: "我要買這個", ja: "これをください", romaji: "Ko-re o ku-da-sa-i" },
                    { zh: "我要Check-in/out", ja: "チェックイン / アウト をお願いします", romaji: "Che-kku-in / a-u-to o o-ne-ga-i shi-ma-su" },
                    { zh: "有會說中文的人嗎？", ja: "中国語を話せる人はいますか？", romaji: "Chū-go-ku-go o ha-na-se-ru hi-to wa i-ma-su ka?" }
                ]
            }
        ];
        ...
        const AttractionView = () => {
            const [subView, setSubView] = useState('attractions');
            return (
                <div className="flex flex-col h-full bg-gray-50">
                    <div className="sticky top-0 z-10 bg-white shadow-sm p-2 flex gap-2">
                        <button onClick={() => setSubView('attractions')} className={`flex-1 px-2 py-2 rounded-xl text-sm font-bold transition-all ${subView === 'attractions' ? 'bg-indigo-600 text-white shadow scale-105' : 'bg-gray-100 text-gray-500'}`}>⛩️ 景點</button>
                        <button onClick={() => setSubView('travel')} className={`flex-1 px-2 py-2 rounded-xl text-sm font-bold transition-all ${subView === 'travel' ? 'bg-teal-600 text-white shadow scale-105' : 'bg-gray-100 text-gray-500'}`}>🚉 資訊</button>
                        <button onClick={() => setSubView('language')} className={`flex-1 px-2 py-2 rounded-xl text-sm font-bold transition-all ${subView === 'language' ? 'bg-orange-600 text-white shadow scale-105' : 'bg-gray-100 text-gray-500'}`}>🗣️ 日文</button>
                    </div>
                                ...
                                {travelInfos.map((info, idx) => (
                                    <div key={idx} className="bg-white border border-gray-100 rounded-2xl p-4 shadow-sm flex flex-col gap-2 mb-4">
                                        <div className="flex items-center gap-2 mb-2">
                                            ...
                                            <h3 className="font-bold text-gray-800 text-[16px]">{info.title}</h3>
                                        </div>
                                        <div className="space-y-3">
                                            {info.items.map((item, iIdx) => (
                                                <div key={iIdx} className="text-[13px] text-gray-600 leading-relaxed bg-gray-50 p-2.5 rounded-lg border border-gray-100">{item}</div>
                                            ))}
                                        </div>
                                    </div>
                                ))}
                            ...
                            <div>
                                ...
                                {japanesePhrases.map((phraseGroup, idx) => (
                                    <div key={idx} className="mb-6">
                                        <h3 className="font-bold text-gray-700 mb-3 ml-1">{phraseGroup.category}</h3>
                                        <div className="bg-white border border-gray-100 rounded-2xl shadow-sm overflow-hidden">
                                            {phraseGroup.words.map((word, wIdx) => (
                                                <div key={wIdx} className={`p-3 flex flex-col gap-1 ${wIdx !== phraseGroup.words.length - 1 ? 'border-b border-gray-50' : ''}`}>
                                                    <div className="flex justify-between items-end">
                                                        <span className="font-bold text-gray-800 text-[15px]">{word.zh}</span>
                                                        <span className="text-orange-600 font-bold text-[14px]">{word.ja}</span>
                                                    </div>
                                                    <div className="text-right text-gray-400 text-[11px] font-mono">{word.romaji}</div>
                                                </div>
                                            ))}
                                        </div>
                                    </div>
