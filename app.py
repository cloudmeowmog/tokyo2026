...
def get_img_as_base64(file_path):
    """
    讀取本機圖片並轉為 Base64 字串，讓 HTML 可以直接顯示
    """
    if not os.path.exists(file_path):
        return "" # 如果找不到圖，回傳空字串避免報錯
    with open(file_path, "rb") as f:
        data = f.read()
    return base64.b64encode(data).decode()

# --- 3. 讀取圖片 ---
# 請確保這三張圖跟 app.py 在同一個資料夾
img_trip = f"data:image/jpeg;base64,{get_img_as_base64('trip.jpg')}"
img_note = f"data:image/jpeg;base64,{get_img_as_base64('note.jpg')}"
img_map  = f"data:image/jpeg;base64,{get_img_as_base64('map.jpg')}"
...
# --- 5. 嵌入 HTML (使用 Python f-string 注入圖片編碼) ---
html_code = f"""
<!DOCTYPE html>
<html lang="zh-TW">
<head>
...
<body class="h-screen flex flex-col overflow-hidden text-gray-800">
    ...
    <script type="text/babel">
        ...
        // 接收 Python 傳進來的圖片 Base64
        const IMG_TRIP = "{img_trip}";
        const IMG_NOTE = "{img_note}";
        const IMG_MAP = "{img_map}";
                    ...
                    <div className="flex-1 flex flex-col items-center w-full">
                        {{mode === 'attraction' && (
                            <div className="w-full max-w-sm bg-blue-50 rounded-xl overflow-hidden shadow-inner border-2 border-blue-100 p-0">
                                <img src={{IMG_TRIP}} alt="行程全覽地圖" className="w-full h-auto" />
                            </div>
                        )}}
                        ...
                        {{mode === 'metro' && (
                            <div className="w-full max-w-sm bg-white rounded-xl overflow-hidden shadow-inner border-2 border-gray-200 p-0">
                                <img src={{IMG_NOTE}} alt="路線手稿" className="w-full h-auto" />
                            </div>
                        )}}
                            ...
                            <div className="w-full max-w-sm">
                                <div className="bg-white rounded-xl overflow-hidden shadow border p-1 mb-4">
                                     <div ref={{mapContainerRef}} className="overflow-auto h-[60vh]">
                                        <img src={{IMG_MAP}} alt="完整地鐵圖" className="w-auto h-full min-w-[200%] object-contain" />
                                     </div>
